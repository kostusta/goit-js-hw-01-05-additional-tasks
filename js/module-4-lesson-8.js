console.log('** Module 4. Lesson 8. Перебирающие методы массива **')
...
// const getModels = cars => {return cars.map(({model}) => model)};
...
// const makeCarsWithDiscount = (cars, discount) => {return cars.map((car) => ({...car, price: car.price - car.price * discount}))};
...
// const sortByAscendingAmount = cars => [...cars].sort((firstCar, srcondCar) => firstCar.amount - srcondCar.amount);
...
// const sortByDescendingPrice = cars => [...cars].sort((firstCar, srcondCar) => srcondCar.price - firstCar.price);
...
// // названию модели в алфавитном и обратном алфавитном порядке, в засисимости от 
...
// const getSortedCarsOnSale = cars => cars.filter(({ onSale }) => onSale).sort((firstCat, secondCar) => firstCat.price - secondCar.price);
